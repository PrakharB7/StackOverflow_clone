...
export const addFriend = async (req, res) => {
  ...
  try {
    await User.findByIdAndUpdate(_id, {
      $addToSet: { friendRequests: [{ requestedId, requestedName }] },
    });
...
export const acceptRequest = async (req, res) => {
  ...
  try {
    await User.findByIdAndUpdate(_id, {
      $addToSet: { friends: [{ friendId, friendName }] },
    });
    await User.findByIdAndUpdate(friendId, {
      $addToSet: { friends: [{ friendId: _id, friendName: userName }] },
    });
...
export const removeFriend = async (req, res) => {
  ...
  try {
    ...
    await User.updateOne({ _id }, { $pull: { friends: { _id: objId } } });
